...
- **État en ligne** : **v39** (25/09/2026, poussée par Claude Code ; avant : 37.1 `4b35c81` correctif de sécurité, v38 `4277f6a`). Phases faites : 1 à 12, 14, 15, 17, 18 (dont notes privées et feedback), 19. Restent : **16, 13** — 13 touche la base (Storage) — puis **20** (tests complets).
...
- Pas de bucket Storage. Advisors : `est_coach()` exposé en RPC (sans risque), protection « mots de passe compromis » désactivée (réglage dashboard).
...
### C. Phase 16 (locked states) — sans base, après 15
Dans `afficher()` : si `Auth.estProspect()` et `outil.verrou` et pas de consultation → rendre l'en-tête + `UI.verrou({titre, texte: "Cette fonctionnalité est disponible avec l'accompagnement MHX.", lien: CONFIG.marque.calendly /* à ajouter dans CONFIG.marque : "https://calendly.com/mhx-coaching/30min" */, cta: "Réserver mon appel"})` et ne pas appeler `init()`. Verrouillés : `programme`, `nutrition`, `mensurations`, `suivi`, `bilan`, `complements`. Ouverts : `accueil` (version prospect : bienvenue, CTA formation, liste de ce que débloque l'accompagnement, CTA Calendly), `formation`, `profil`. Cadenas dans la nav pour les onglets verrouillés. Entrées `I18N.en` déjà présentes : « Cette fonctionnalité est disponible avec l'accompagnement MHX. », « Réserver mon appel ». **Aucun prix.**

### D. Phase 13 (photos) — Storage
...
- **Avant d'ouvrir l'inscription publique** (Lucas, à la fin) : (1) la phase 16 doit être en ligne (en v39 seule, un prospect a exactement l'espace d'un client) ; (2) la robustesse de `Clients.resumer` face aux données malformées (phase 20) doit être faite — sinon un inconnu inscrit pourrait faire tomber le tableau de bord du coach ; (3) activer la **confirmation d'email** dans Supabase (aujourd'hui désactivée : sinon n'importe qui pourrait s'inscrire avec l'adresse de quelqu'un d'autre ; sans risque pour les comptes existants, tous confirmés) ; un **CAPTCHA** est un chantier de code à faire AVANT (widget + `gotrue_meta_security.captcha_token` sur l'inscription, la connexion et « mot de passe oublié ») : **ne jamais activer le CAPTCHA seul dans Supabase**, il vaut aussi pour la connexion et bloquerait tous les clients ; (4) savoir que le verrouillage du mode gratuit est un verrou d'**interface** : un prospect ne possède que ses propres lignes (RLS), il n'a donc rien de privé à atteindre, mais il peut techniquement écrire ses propres clés par l'API, comme un client. Contrôle après chaque ouverture ou création de compte : `select role, statut, count(*) from profils group by 1, 2`.
...
1. Cloner `https://github.com/lcsmhx/mhx-plateforme` ; vérifier que `CONFIG.marque.version` du dépôt et le pied de page de https://lcsmhx.github.io/mhx-plateforme/ affichent la même version (v39 au 25/09/2026).
2. Lire `NOTESCLAUDE.md` en entier (sections du 25/09/2026 : v33 → v39).
3. Installer le banc de test (`tests-locaux/README.md`) et le lancer sur la version en ligne : attendu v39 = 64 pages, 0 erreur, 19 + 13 + 14 + 13 + 15 + 67 vérifications, verif-xss 5/5, verif39 34/34, toutes réussies (voir §8).
